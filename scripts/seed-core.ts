/**
 * シードコア（ローカル D1 / リモート D1 共通）— 冪等
 * ユーザー/インシデント/問題/変更/CMDB/ナレッジ/資産/パッチ/セキュリティ/要求を投入する。
 */
import { hashPassword } from '../src/auth.ts';
import { D1Client } from '../src/db/client.ts';
import type { D1Like } from '../src/db/client.ts';

export async function runSeed(dbLike: D1Like, env: Record<string, string>): Promise<void> {
  const db = new D1Client(dbLike);
  ...
  for (const u of users) {
    const id = await seedUser(db, { ...u, role: u.role }, password);
...
async function seedUser(db: D1Client, u: { username: string; display_name: string; email: string; role: string; department: string }, password: string) {
...
async function seedIfEmpty(db: D1Client, table: string, rows: { sql: string; params: unknown[] }[]) {
  const count = await db.queryOne<{ total: number }>(`SELECT COUNT(*) AS total FROM ${table}`);
  if (count?.total) {
